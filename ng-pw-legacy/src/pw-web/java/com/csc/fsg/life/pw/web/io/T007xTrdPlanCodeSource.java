...
public class T007xTrdPlanCodeSource
	extends TCDynamicTarget
{
	private static Log log = PWServerLogManager.getLog(T007xTrdPlanCodeSource.class.getPackage().getName());
	...
	private List<String[]> getAvValues(String envId, PlanTO plan, Connection connection)
	{
		...
		String query = new StringBuilder()
			.append("SELECT DISTINCT TT06T.RIDER_BENE_TYPE,")
			.append("                T000X.PLAN_CODE")

			.append(" FROM  " + schemaName + ".T000X T000X,") 
			.append("       " + schemaName + ".TT06T TT06T ")

			.append("WHERE  T000X.COMPANY_CODE = TT06T.COMPANY_CODE") 
			.append("  AND  T000X.TABLE_PTR_SUBSET = TT06T.TABLE_SUBSET")
			.append("  AND  T000X.PLAN_TYPE = 'R'")
			.append("  AND  T000X.PRODUCT_PREFIX = 'T'") 
			.append("  AND  T000X.TABLE_PTR_ID = 'T06'")
