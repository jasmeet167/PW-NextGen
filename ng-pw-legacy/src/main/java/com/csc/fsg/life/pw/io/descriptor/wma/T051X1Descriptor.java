    ...
    extends TableDescriptor
{
    ...
    public void initializeColumnDescriptors()
    {
        ...
        addColumnDescriptor(new ColumnDescriptor(this,"getIntRate","setIntRate","INT_RATE,3,4,5,3,false|,0,DOUBLE,0,null,null,null,null,null"));
    ...
    public String getPagingSQL(String schemaName,boolean isSubsetMode,boolean isNext, boolean locator)
    {
        ...
        if (isNext) {
            if (isSubsetMode)
                if (locator)
                    pagingWhere = ".T051X1 WHERE (COMPANY_CODE = :company_code) AND (TABLE_SUBSET = :table_subset) AND ((INTEREST_RATE_SEQ > :interest_rate_seq) OR (INTEREST_RATE_SEQ = :interest_rate_seq)) ";
                else 
                    pagingWhere = ".T051X1 WHERE (COMPANY_CODE = :company_code) AND (TABLE_SUBSET = :table_subset) AND ((INTEREST_RATE_SEQ > :interest_rate_seq)) ";
            else
                if (locator)
                    pagingWhere = ".T051X1 WHERE (COMPANY_CODE = :company_code) AND ((TABLE_SUBSET > :table_subset) OR (TABLE_SUBSET = :table_subset AND INTEREST_RATE_SEQ > :interest_rate_seq) OR (TABLE_SUBSET = :table_subset AND INTEREST_RATE_SEQ = :interest_rate_seq)) ";
                else
                    pagingWhere = ".T051X1 WHERE (COMPANY_CODE = :company_code) AND ((TABLE_SUBSET > :table_subset) OR (TABLE_SUBSET = :table_subset AND INTEREST_RATE_SEQ > :interest_rate_seq)) ";
