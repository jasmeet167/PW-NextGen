...
public class XmlWriter {
	...
	public XmlWriter(Document doc, boolean xmlDeclaration) {
		this(doc, null, null);
	 ...
	 * 
	 * @param node
	 *            The node to get the next sibling for.
	...
	public void output(File outFile, String indentChar) {
	 ...
	 * 
	 * @param indentBuf
	 *            Output StringBuffer containing XML
	 */
		...
		case Node.DOCUMENT_NODE: {
			...
			if (docType != null) {
				...
				if (docType.getPublicId() != null) {
					xmlBuf.append(" PUBLIC \"");
					xmlBuf.append(docType.getPublicId());
					xmlBuf.append("\"");
				}
...
//			if (docElement.getNodeName().equals(node.getNodeName()) && uri != null) {
...
//			}
			...
			for (int i = attrs.getLength() - 1; i >= 0; i--) {
				...
				if ((!attrName.startsWith("xmlns")) || (attrValue.trim().length() > 0)) {
					xmlBuf.append(' ');
